...
public class QueryBuilder {
    ...
    public String build() {
        ...
        if (this.id != 0) {
            resBuff.append(StrCodec.getParamStr(PARAM_ID, this.id.toString())).append(AMPERSAND_SIGN);
        }
        if (this.name != null) {
            resBuff.append(StrCodec.getParamStr(PARAM_NAME, this.name)).append(AMPERSAND_SIGN);
        }
        if (this.genre != null) {
            resBuff.append(StrCodec.getParamStr(PARAM_GENRE, this.genre)).append(AMPERSAND_SIGN);
        }
        if (this.author != null) {
            resBuff.append(StrCodec.getParamStr(PARAM_AUTHOR, this.author)).append(AMPERSAND_SIGN);
        }
        if (this.year_of_issue != 0) {
            resBuff.append(StrCodec.getParamStr(PARAM_Y_OF_ISSUE, this.year_of_issue.toString())).append(AMPERSAND_SIGN);
        }
        if (this.link != null ) {
            resBuff.append(StrCodec.getParamStr(PARAM_LINK, this.link)).append(AMPERSAND_SIGN);
        }
        if (resBuff.length()>0) {
